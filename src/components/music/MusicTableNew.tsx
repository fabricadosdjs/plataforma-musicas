...
interface MusicTableNewProps {
    tracks: Track[];
    onDownload?: (tracks: Track[]) => void;
    isDownloading?: boolean;
}

const MusicTableNew = ({ tracks, onDownload, isDownloading }: MusicTableNewProps) => {
    ...
    // Carrega likes do usuário ao montar
    useEffect(() => {
        if (!user?.id) return;
        fetchLikes();
    }, [user?.id]);
        ...
        <div className="relative w-full h-full">
            ...
            >
                ...
                <table className="hidden md:table min-w-full text-left text-sm text-gray-200 table-fixed" style={{ fontFamily: "'Lato', sans-serif", fontSize: '13px' }}>
                    ...
                    <tbody className="divide-y divide-gray-800" style={{ fontFamily: "'Lato', sans-serif" }}>
                        ...
                        {tracks.map((track: Track) => (
                            <tr
                                key={track.id}
                                className="hover:border-l-4 hover:border-l-gray-600 transition-all duration-300"
                            >
                                    ...
                                    {session ? (
                                        <div className="flex flex-row flex-nowrap items-center justify-end gap-1">
                                            ...
                                            <button
                                                ...
                                                className="inline-flex items-center justify-center p-2 rounded-lg text-xs font-bold transition-all duration-300 cursor-pointer tracking-wide shadow-lg bg-yellow-700 text-white hover:bg-yellow-800 border border-yellow-500 shadow-yellow-500/25"
                                            ...
                                            <button
                                                ...
                                                className="inline-flex items-center justify-center p-2 rounded-lg text-xs font-bold transition-all duration-300 cursor-pointer tracking-wide shadow-lg bg-gray-800 text-white hover:bg-purple-800 border border-purple-500 shadow-purple-500/25"
                ...
                <div className="md:hidden flex flex-col gap-4 p-2">
                    ...
                    {tracks.map((track: Track) => (
                        <div key={track.id} className="rounded-2xl bg-gradient-to-br from-gray-900 to-black border border-gray-800 shadow-xl p-4 flex flex-col gap-3">
                            <div className="flex items-center gap-4">
