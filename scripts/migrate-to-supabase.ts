...
async function migrateAllData() {
    ...
    ` as any[]
        ...
        for (const tableInfo of tables) {
            ...
            try {
                // Contar registros
                const count = await sourcePrisma.$queryRaw`SELECT COUNT(*) as count FROM "${tableName}"` as any[]
                ...
                if (recordCount > 0) {
                    ...
                    for (let offset = 0; offset < recordCount; offset += batchSize) {
                        const records = await sourcePrisma.$queryRaw`SELECT * FROM "${tableName}" LIMIT ${batchSize} OFFSET ${offset}` as any[]
